...
if($pseudo!=""){
	...
	$query='SELECT id FROM utilisateur WHERE pseudo="'.$pseudo.'"';
...
if (isset($_POST['message']) && !empty($_POST['message'])) {
	...
	if(!isset($_POST['id']) || empty($_POST['id'])){
		$query = 'INSERT INTO messages (contenu, date, user_id) VALUES (:contenu, UNIX_TIMESTAMP(),:id_utilisateurs)';
		$prep = $pdo->prepare($query);
	}
	else{
		$query = 'UPDATE messages set contenu=(:contenu), date= UNIX_TIMESTAMP(), user_id=(:id_utilisateurs) WHERE id=(:id)';
		$prep = $pdo->prepare($query);
